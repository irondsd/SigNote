...
export function makeVersionsRouter<R>(opts: {
  getById: (userId: string, id: string) => Promise<Ownable | null>;
  getVersions: (userId: string, id: string) => Promise<WithVersions | null>;
  deleteVersion: (userId: string, id: string, versionId: string) => Promise<unknown | null>;
  restoreVersion: (userId: string, id: string, versionId: string) => Promise<R | null>;
  ...
  return router({
    ...
    list: protectedProcedure.input(z.object({ id: objectId })).query(async ({ ctx, input }) => {
      const doc = assertOwner(await opts.getVersions(ctx.userId, input.id), ctx.userId);
      return doc.versions ?? [];
    }),
    ...
    delete: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        return withVaultWrite(ctx.userId, async () => {
          assertOwner(await opts.getById(ctx.userId, input.id), ctx.userId);
          const updated = await opts.deleteVersion(ctx.userId, input.id, input.versionId);
    ...
    restore: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        return withVaultWrite(ctx.userId, async () => {
          assertOwner(await opts.getById(ctx.userId, input.id), ctx.userId);
          const updated = await opts.restoreVersion(ctx.userId, input.id, input.versionId);

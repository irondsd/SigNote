...
function PaletteRow({
  ...
  return (
    <button type="button" className={cn(s.row, active && s.rowActive)} onMouseDown={(e) => e.preventDefault()} onClick={onClick}>
...
export function AddTagButton({ value, onChange }: AddTagButtonProps) {
            ...
            <>
              {recent.length > 0 && (
                <>
                  ...
                  {recent.map((t, i) => (
                    <PaletteRow key={t._id} tag={t} count={counts[t._id]} active={i === 0} showEnter={i === 0} onClick={() => add(t._id)} />
                  ))}
                </>
              )}
              {rest.length > 0 && (

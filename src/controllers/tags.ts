...
export async function touchTags(userId: string, ids: string[]): Promise<void>;
export async function touchTags(ids: string[]): Promise<void>;
export async function touchTags(userOrIds: string | string[], maybeIds?: string[]): Promise<void> {
  const userId = typeof userOrIds === 'string' ? userOrIds : null;
  const ids = typeof userOrIds === 'string' ? (maybeIds ?? []) : userOrIds;
  if (ids.length === 0) return;
  const write = async () => {
    try {
      await getDb().update(tags).set({ lastUsedAt: new Date() }).where(inArray(tags.id, ids));
    } catch (err) {
      // Usage tracking is best-effort; a failure here must not fail the note
      // save. Fencing conflicts are security state, though, and must escape.
      if (err instanceof Error && err.name === 'VaultConflictError') throw err;
    }
  };
  if (userId) await withVaultWrite(userId, write);
  else await write();
}

export const getTagById = async (id: string, userId?: string): Promise<TagRow | null> => {
  const read = async () => {
    const rows = await getDb().select().from(tags).where(eq(tags.id, id)).limit(1);
    return rows[0] ? mapTag(rows[0]) : null;
  };
  if (userId) return withVaultRead(userId, read);
  const row = await read();
  return row ? withVaultRead(row.userId, read) : null;
};
...
export async function updateTag(
  ...
  const existing = userId ? null : await getTagById(id);
  const owner = userId ?? existing?.userId;
  if (!owner) return null;
  return withVaultWrite(owner, async () => {
    const rows = await getDb()
      .update(tags)
      .set({ ...patch, updatedAt: new Date() })
      .where(and(eq(tags.id, id), eq(tags.userId, owner)))
      .returning();
    return rows[0] ? mapTag(rows[0]) : null;
  });
}
...
export async function deleteTagAndDetach(id: string, userId?: string): Promise<void> {
  const existing = userId ? null : await getTagById(id);
  const owner = userId ?? existing?.userId;
  if (!owner) return;
  await withVaultWrite(owner, async () => {
    await getDb()
      .delete(tags)
      .where(and(eq(tags.id, id), eq(tags.userId, owner)));
  });
